classdef ImageUtil
    ...
    methods (Static)
        ...
        function [x1, y1, x2, y2] = lineDetection(image, debug)
            I = image;
            %transforms into array of arrays
            I = im2gray(I(:,:,1));
            %level is threshold for determining b/w for binarized img
            %higher level=brighter needed for white in bin img
        ...
        %generates white circles of cell outlines
        function [bw4, thresh] = getCleanedBW( image, debug, ftitle, thresh)
            %enhances contrast of grayscale
            I_eq = adapthisteq(image);
            %calcs threshold using Otsu's method
            if(isnan(thresh))
        ...
        function cellCentroid = locateCentroids(image, debug, label)
            %calls getCleanedBW to make mask of all cells
            [bwmask, ~]= ImageUtil.getCleanedBW(image, false, label, NaN);
            %given bwmask, regionprops calcs centroid and weightedCentroid
            %weightedCentroid not used
            %info stored in array s (n elements, ea obj w/ 2 fields)
            s = regionprops(bwmask,image,{'Centroid','WeightedCentroid'});
            %initializing for performance benefit, prevents constantly
            %creating new arrays
            numObj= numel(s);
            weightedCellCentroid=zeros(numObj, 2);
            cellCentroid=zeros(numObj, 2);
            %for kth struct of s w/ field for Centroid, takes val of
            %field's kth row and puts into kth row of prev initialized
            %array
            %essentially transcribes all centroid coords from s into
            %cellCentroid
            for k = 1 : numObj
